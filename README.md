...
Users enqueue edge operations (`ADD` or `REVOKE`) into an on-chain queue. A batcher (“forger”) selects the next `n` queued ops (`n <= MAX_BATCH`), constructs `txData`, proves correct state transition in a zkSNARK, and submits a proof to update the on-chain `latestGraphRoot`.

The contract verifies a Groth16 proof against a single public input:
- `pub0 = mask253( sha256(latestRoot, newRoot, batchId, start, n, storageHash) )`

Where:
- `storageHash = sha256(txData)` (packed encoding)
- `txData` is `MAX_BATCH * 9` bytes, one fixed-size record per queued op
